...
function applyTheme(t){
  ...
  const meta=document.querySelector('meta[name="theme-color"]'); if(meta) meta.setAttribute('content', State.theme==='dark'?'#1C1B1F':'#FAFAF8');
...
async function loadDeckProgress(){
  const {data,error}=await sb.from('deck_progress').select('jlpt_level,kanji,status,last_reviewed_at');
  State.deckProgress={}; State.deckDates={};
  if(!error){ (data||[]).forEach(r=>{ const k=r.jlpt_level+'|'+r.kanji; State.deckProgress[k]=r.status; State.deckDates[k]=r.last_reviewed_at; }); }
}
// Jumlah kata deck yang ditandai hafal hari ini (untuk aktivitas)
function deckTodayCount(level){
  const today=todayStr(); let n=0; const pre=level+'|';
  for(const k in State.deckDates){ if(k.startsWith(pre)&&State.deckProgress[k]==='hafal'&&State.deckDates[k]&&String(State.deckDates[k]).slice(0,10)===today) n++; }
  return n;
}
...
async function createNote(o){ const {data,error}=await sb.from('notes').insert({user_id:State.user.id,category_id:o.category_id||null,title:o.title,content:o.content||'',image_url:o.image_url||null}).select().single(); if(error){toast('Gagal menyimpan catatan','error');return null;} State.notes.unshift(data); return data; }
...
function addRecentLookup(word){
  if(!word) return;
  let r=getRecentLookup().filter(x=>x!==word); r.unshift(word); r=r.slice(0,8);
  localStorage.setItem('nf_recent_lookup',JSON.stringify(r)); renderRecentLookup();
}
function renderRecentLookup(){
  const r=getRecentLookup(); const wrap=$('#lk-recent'); if(!wrap) return;
  if(!r.length){ wrap.classList.add('hidden'); return; }
  wrap.classList.remove('hidden');
  $('#lk-recent-chips').innerHTML=r.map(w=>`<button class="lk-recent-chip" data-recent="${esc(w)}">${esc(w)}</button>`).join('');
  $$('#lk-recent-chips [data-recent]').forEach(b=>b.onclick=()=>{ $('#lk-input').value=b.dataset.recent; doLookup(); });
}
function bindLookup(){
  $('#lk-btn')?.addEventListener('click',doLookup);
  $('#lk-input')?.addEventListener('keydown',e=>{ if(e.key==='Enter'){ e.preventDefault(); doLookup(); } });
  $('#lk-add')?.addEventListener('click',()=>{ if(_lkData) addDictToFlashcard(_lkData); });
  $('#lk-fav')?.addEventListener('click',()=>{ if(_lkData) lookupAddFav(_lkData); });
  renderRecentLookup();
}
async function doLookup(){
  const q=$('#lk-input')?.value?.trim(); if(!q){ toast('Ketik kata dulu','warning'); return; }
  $('#lk-result')?.classList.add('hidden'); $('#lk-empty')?.classList.add('hidden');
  $('#lk-loading')?.classList.remove('hidden');
  try{
    let hit=await lookupDictionary(q);   // cek kamus dulu
    if(!hit){
      try{ const {reading,meaning}=await fetchJapanese(q); hit={kanji:q,reading,meaning}; }
      catch(e){ hit=null; }
    }
    if(hit){ _lkData=hit; showLookup(hit); addRecentLookup(hit.kanji||q); }
    else { $('#lk-empty')?.classList.remove('hidden'); }
  }catch(err){ toast('Gagal mencari: '+err.message,'error',3500); }
  finally{ $('#lk-loading')?.classList.add('hidden'); }
}
function showLookup(d){
  $('#lk-kanji').textContent=d.kanji||'';
...
async function lookupAddFav(d){
  ...
  const ok=await modal({title:'Tambah ke Favorit',
...
function obRender(){
  ...
  if(next){
    ...
    if(idx === OB_TOTAL-1){ next.style.background='var(--primary)'; }
...
function renderDashboard(){
  ...
  if($('#dash-streak')) $('#dash-streak').textContent='🔥 '+streak;
  // Ringkasan Hari Ini
  ...
  // Hari Ini card (review + kata baru + estimasi)
  if($('#today-review')) $('#today-review').textContent=due;
  if($('#today-new')) $('#today-new').textContent=baru;
  if($('#today-time')){ const mins=Math.max(1,Math.round((due+baru)*0.25)); $('#today-time').innerHTML=mins+'<span style="font-size:12px"> mnt</span>'; }
  ...
  // Review banner — dengan empty state
  const banner=$('#review-banner');
  if(due>0){
    banner.innerHTML=`<div class="banner" style="margin-bottom:14px"><div class="grow"><div class="bt">📚 Ada ${due} kartu untuk direview.</div></div><button class="btn btn-sm btn-primary" id="banner-review">Review →</button></div>`;
    $('#banner-review').onclick=()=>goto('review');
  } else {
...
async function renderDashDecks(){
  const wrap=$('#dash-decks'); if(!wrap) return;
  if(!State.deckTotals){
    wrap.innerHTML=`<div class="empty" style="padding:20px"><span class="em">⏳</span>Memuat deck…</div>`;
    try{
      State.deckTotals={};
      for(const lv of ['N5','N4','N3']){
        const {count}=await sb.from('dictionary').select('id',{count:'exact',head:true}).eq('jlpt_level',lv);
        State.deckTotals[lv]=count||0;
      }
    }catch(e){ State.deckTotals=null; wrap.innerHTML=`<div class="empty" style="padding:16px">Gagal memuat deck</div>`; return; }
  }
  const names={N5:'Dasar',N4:'Menengah',N3:'Lanjutan'};
  wrap.innerHTML=['N5','N4','N3'].map(lv=>{
    const total=State.deckTotals[lv]||0;
    const hafal=Math.min(deckHafalCount(lv),total);
    const pct=total?Math.round(hafal/total*100):0;
    const today=deckTodayCount(lv);
    ...
    return `<div class="deck-card" data-deck="${lv}">
      ...
      <div class="deck-info">
        ...
        <div class="deck-pct"><b>${pct}% Selesai</b> · ${hafal} kata dipelajari${act}</div>
      </div>
      <div class="deck-go">›</div>
    </div>`;
  }).join('');
  // Animasi fill saat dibuka
  requestAnimationFrame(()=>{ $$('.deck-pfill',wrap).forEach(f=>{ f.style.width=(f.dataset.pct||0)+'%'; }); });
...
function drawWeekChart(){
  ...
  const barActive=isDark?'#E892A2':'#DE7A8C';
  const barPassive=isDark?'#3F3F46':'#FCECEF';
...
async function deleteAvatar(){
  if(!State.user||!State.profile?.avatar_url) return;
  const ok=await modal({title:'Hapus Foto Profil?',body:`<p>Foto profil akan dihapus.</p>`,confirmText:'Hapus',danger:true});
  if(!ok) return;
  overlay('Menghapus foto…');
  try{
...
function fmtDate(d){ if(!d)return ''; try{ return new Date(d).toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'}); }catch(e){ return ''; } }
function renderNotes(){
  const arr=visibleNotes();
  const photos=arr.filter(n=>n.image_url);
  const manuals=arr.filter(n=>!n.image_url);
  // Photo gallery
  const pwrap=$('#note-photos');
  if(pwrap){
    pwrap.innerHTML = photos.length ? `<div class="note-grid">`+photos.map(n=>`
      <div class="note-photo-card">
  ...
  $$('[data-del-note]').forEach(b=>b.onclick=()=>confirmDeleteNote(b.dataset.delNote));
  $$('[data-view]').forEach(img=>img.onclick=()=>openLightbox(img.dataset.view));
}
function openLightbox(url){
  const div=document.createElement('div'); div.className='lightbox';
  div.innerHTML=`<button class="lb-close">✕</button><img src="${esc(url)}"/>`;
...
async function uploadNoteImage(file){
  ...
  try{
    ...
    const {error:upErr}=await sb.storage.from('notes').upload(path,file,{upsert:false,contentType:file.type});
    if(upErr) throw new Error('Upload gagal: '+upErr.message);
    const {data}=sb.storage.from('notes').getPublicUrl(path);
    const note=await createNote({title:'Foto '+new Date().toLocaleDateString('id-ID'),content:'',image_url:data.publicUrl});
    overlayOff();
    if(note){ toast('Foto tersimpan ✓','success'); renderNotes(); }
  }catch(e){ overlayOff(); toast(e.message,'error',4000); }
}
async function noteForm(id){
  const editing=State.notes.find(n=>n.id===id);
  // Catatan foto → hanya edit judul
  if(editing&&editing.image_url){
    const ok=await modal({title:'Edit Judul Foto',
      body:`<div style="text-align:center;margin-bottom:14px"><img src="${esc(editing.image_url)}" style="max-width:100%;max-height:200px;border-radius:14px"/></div>
        <div class="field"><label>Judul</label><input class="input" id="m-note-title" value="${esc(editing.title||'')}" placeholder="Judul foto"/></div>`,
      confirmText:'Simpan',
      onConfirm:async()=>{ const title=$('#m-note-title').value.trim()||'Tanpa judul'; await updateNote(id,{title}); }});
    if(ok){ toast('Judul diperbarui','success'); renderNotes(); }
    return;
  }
...
function renderProgress(){
  ...
  $('#pg-total').textContent=total; $('#pg-hafal').textContent=hafal; $('#pg-pct').textContent=pct+'%';
  // Streak
  if($('#pg-streak')) $('#pg-streak').textContent=effectiveStreak();
  // Favorit
  if($('#pg-fav')) $('#pg-fav').textContent=State.flashcards.filter(f=>f.favorite).length;
  // Deck selesai (level dengan progress 100%)
  if($('#pg-decks')){
    let done=0;
    if(State.deckTotals){ for(const lv of ['N5','N4','N3']){ const t=State.deckTotals[lv]||0; if(t>0 && deckHafalCount(lv)>=t) done++; } }
    $('#pg-decks').textContent=done;
  }
...
function drawSrs(){
  ...
  cv.width=W*dpr;cv.height=H*dpr; const ctx=cv.getContext('2d'); ctx.scale(dpr,dpr); ctx.clearRect(0,0,W,H);
  const css=getComputedStyle(document.documentElement);
  const bar=(css.getPropertyValue('--primary').trim())||'#DE7A8C';
  const cText=(css.getPropertyValue('--text').trim())||'#111827';
  const cText2=(css.getPropertyValue('--text-2').trim())||'#6B7280';
...
function renderSettings(){
  const name=State.profile?.name||'Pelajar';
  $('#set-name').textContent=name; $('#set-email').textContent=State.user?.email||'';
  // Avatar — pakai foto dari profiles.avatar_url (single source of truth)
  const avatarUrl=State.profile?.avatar_url;
  const setEl=$('#set-avatar');
  if(setEl){
    if(avatarUrl) setEl.innerHTML=`<img src="${avatarUrl}" alt="avatar"><span class="avatar-cam">📷</span>`;
    else setEl.innerHTML=(name[0]||'P').toUpperCase()+'<span class="avatar-cam">📷</span>';
  }
...
function bindEvents(){
  ...
  $('#note-photo-file')?.addEventListener('change',e=>{ const f=e.target.files?.[0]; if(f)uploadNoteImage(f); e.target.value=''; });
  ...
  $('#set-avatar-del')?.addEventListener('click',deleteAvatar);
